...
- 2026-09-09: `PermissionDenied` does not fire on a `PreToolUse` hook-issued deny — measured 2026-09-09: a denied `fly deploy` produced `Hook denied tool use for Bash` / `Bash tool permission denied` in the runtime debug log while `.claude/metrics/` gained no record, so `a5bb8fc`'s logger has now had its first real opportunity and missed it. Root cause is documented behaviour, not a defect: `/en/hooks.mdx` > *PermissionDenied* states it fires only in auto mode and explicitly not when a `PreToolUse` hook blocks a call, so no event covers hook denials at all — either log from inside `pre_bash_deploy.py`, or read `toolDenialKind` from the session transcript, which needs no change to a live guard (see `docs/plans/plan-2026-09-09-transcript-session-review.md`).
...
- 2026-09-09: `ruff check .` reports `E401 Multiple imports on one line` at `.claude/hooks/log_event.py:3` (`import json, os, sys`), introduced by `a5bb8fc` and invisible to the `PostToolUse` hook, which lints only the edited file — cosmetic, so it waits for a change that touches that file on its own merits rather than forcing one.
- 2026-09-09: The cite-by-heading rule has no enforcement — five `HOOKS-REF.md:<n>` citations across `pre_bash_deploy.py`, `post_edit_python.py` and `tests/test_pre_bash_deploy.py` are correct today only because the vendored doc has not been re-vendored since they were written; make it a `check_invariants.py` check that resolves each citation and asserts the target line still contains an expected token, converting prose an agent may not read into a control.
- 2026-09-09: The "Promote to global" trigger on `CLAUDE.md` > *Citing the vendored docs* has fired — the workbench extended past this repo on 2026-09-09 (wrote to `~/.claude/CLAUDE.md`, worked in the sibling `mcpfleet-obs`, surveyed the fleet), so move that rule to the global file where it loads in all twelve repos; note the section itself is still an uncommitted working-tree edit, so it currently binds nothing on a fresh clone.
- 2026-09-09: The `mcpfleet-obs` validation-label fix of 2026-09-06 is still uncommitted in that repo (23 tests pass across the 3.2.3/3.2.4/3.3.0/3.4.5 matrix, nothing tagged, PyPI still 0.1.0), so prod runs the unfixed taxonomy and schema rejections still land in `unknown`; publishing 0.2.0 will not by itself reach prod, since `pyproject.toml` floats `>=0.1.0` while `Dockerfile` hard-pins `==0.1.0` — the release needs both changed together.
- 2026-09-09: `vat_validate` is dead on prod with credentials set-but-rejected, not missing — 9 `configuration` errors and zero successes since the counter's 2026-09-07T21:03Z `_created`, a restart that left code at v1.3.0, so a secrets change did not fix it; the counter cannot separate the two causes but the histogram can, with an empty `le=0.1` bucket and 0.209s mean proving every call reached HMRC and was refused. Suspect `HMRC_ENV`, defaulted to `production` against sandbox credentials.
